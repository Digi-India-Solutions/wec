...
export default function Login() {
    ...
    // Mock users for demo
    const mockUsers = [
        { id: '1', name: 'Super Admin', email: 'admin@amcmanagement.com', role: 'admin' },
        { id: '2', name: 'ABC Electronics Pvt Ltd', email: 'contact@abcelectronics.com', role: 'distributor' },
        { id: '3', name: 'Tech Store Mumbai', email: 'owner@techstoremumbai.com', role: 'retailer' }
    ];
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // console.log("respons:==>", respons.data.user)
            if (respons.status === true) {
                // login(respons.data.user);
    ...
    const handleQuickLogin = async (userEmail, userRole) => {
        setLoading(true);
        // Auto-fill form
        setFormData({
            email: userEmail,
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="relative">
            ...
            <Input label="Email" type="email" value={formData.email} onChange={(e) => setFormData(prev => ({ ...prev, email: e.target.value }))} placeholder="Enter your email" icon="ri-mail-line" required/>

            <Input label="Password" type="password" value={formData.password} onChange={(e) => setFormData(prev => ({ ...prev, password: e.target.value }))} placeholder="Enter your password" icon="ri-lock-line" required/>
          </div>
        ...
        {/* <div className="mt-8">
          ...
          <div className="mt-6 space-y-3">
            {mockUsers.map((user) => (<button key={user.id} type="button" onClick={() => handleQuickLogin(user.email, user.role)} disabled={loading} className="w-full flex items-center justify-between p-3 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">
                ...
                user.role === 'distributor' ? 'ri-building-line' :
                    'ri-store-line'} w-4 h-4 flex items-center justify-center`}></i>
                  </div>
