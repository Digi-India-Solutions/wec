...
export default function ClaimsPage() {
  ...
  const claimFields = [
    ...
    { name: 'amcNumber', label: 'AMC No. / Contract ID', type: 'text' as const, required: true },
    { name: 'claimValue', label: 'Claim Value (₹)', type: 'number' as const, required: true },
    { name: 'productDetails', label: 'Product Details', type: 'textarea' as const, required: true },
    { name: 'billPhoto', label: 'Upload Bill Photo', type: 'file' as const, required: false, accept: '.jpg,.jpeg,.png,.pdf' },
    { name: 'accountHolderName', label: 'Account Holder Name', type: 'text' as const, required: true },
    { name: 'bankName', label: 'Bank Name', type: 'text' as const, required: true },
    { name: 'accountNumber', label: 'Account Number', type: 'text' as const, required: true },
    { name: 'ifscCode', label: 'IFSC Code', type: 'text' as const, required: true },
    { name: 'remarks', label: 'Remarks', type: 'textarea' as const, required: false },
  ...
  const columns = [
    ...
    { key: 'claimValue', title: 'Claim Value', render: (value: number) => `₹${value.toLocaleString()}` },
    { key: 'billPhoto', title: 'Bill Photo', render: (value: string) => {
      if (!value) return '-';
  ...
  const handleSubmit = async (formData: any) => {
    ...
    try {
      ...
      if (editingClaim) {
        setClaims(prev => prev.map(c => c.id === editingClaim.id ? { ...c, ...formData } : c));
        ...
        const newClaim = {
          ...
          ...formData,
          billPhoto: formData.billPhoto || `bill_photo_${Date.now()}.jpg`,
          uploadedBill: 'bill_sample.pdf',
          uploadedCheque: 'cheque_sample.pdf',
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-semibold text-gray-900">{claims.filter(c => c.status === 'pending').length}</p>
            </div>
          </div>
        </div>
        <div className="bg-white p-6 rounded-lg shadow">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-semibold text-gray-900">{claims.filter(c => c.status === 'approved').length}</p>
            </div>
          </div>
        </div>
        <div className="bg-white p-6 rounded-lg shadow">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-semibold text-gray-900">{claims.filter(c => c.status === 'rejected').length}</p>
            </div>
          </div>
        </div>
      </div>
